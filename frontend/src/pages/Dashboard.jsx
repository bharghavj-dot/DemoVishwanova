    ...
    <div className="min-h-screen bg-clinical-bg">
      ...
      <div className="bg-gradient-to-r from-primary-50 via-clinical-bg to-white py-12 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-clinical-muted text-base">
            Your physiological baseline remains stable. Next diagnostic window in 4 hours.
          </p>
          <div className="flex gap-3 mt-6">
            <Link to="/profile" className="btn-ghost flex items-center gap-2 text-sm border border-clinical-border rounded-xl px-4 py-2">
            ...
            {reports.length > 0 && (
              <button className="btn-ghost flex items-center gap-2 text-sm border border-clinical-border rounded-xl px-4 py-2">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        {/* Diagnostic Scan Modules */}
        <h2 className="section-title mb-6">Diagnostic Scan Modules</h2>
        <div className="grid md:grid-cols-3 gap-6 mb-12">
          {modules.map((mod, i) => (
            <div key={i} className="card p-8 group">
              <div className="mb-5">{mod.icon}</div>
              <h3 className={`text-xl font-bold ${mod.color} mb-2`}>{mod.title}</h3>
              <p className="text-sm text-clinical-muted mb-6">{mod.desc}</p>
              <button
                onClick={() => navigate('/scan')}
                className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-primary-500 group-hover:gap-3 transition-all"
              >
                Launch Module
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {reports.slice(0, 6).map((report, i) => (
                <Link
                  ...
                  className="card-static p-5 hover:shadow-clinical-lg transition-all group"
                >
                  ...
                  <h4 className="font-semibold text-clinical-text group-hover:text-primary-500 transition-colors">{report.primary_disease}</h4>
